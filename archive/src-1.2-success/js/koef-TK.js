        ...
        <div class="ki_input_area">
          <div class="ki_btn_zone">
            ...
            <div class="ki_payload_input">
              <button type="button" class="btn btn-outline-primary btn-sm" onclick="document.getElementById('my_ki_file_input_${i}').click()">Файл протокола .xlsx</button>
...
function pickProtocol(oEvent, tempInd) {
  ...
  reader.onload = function(e) {
    ...
    const accureTemp = getAccureTemp(workbook);
    const errorRateObj = getErrorRateObj(workbook, deviceNumbInProto);
...
function getErrorRateObj(workbook, deviceNumb) {
  ...
  let resObjInd = 0;
  debugger
  // Необходимо найти исходную точку путем обхода по линии А.
  let AInd = 1;
  while(1) {
    const key = 'A'+AInd;
